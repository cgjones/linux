...
static int FreeMemoryForHandle(GEMEMALLOC_HDL hdl );


static int gememalloc_open(GEMEMALLOC_HDL *hdlp)
{

	PDEBUG(KERN_DEBUG "gememalloc_open: Current OpenCounter: %d\n", openCounter);
	*hdlp = openCounter++;
	return 0;
}
				...
				unsigned int phy_start_address)
{
    ...
    memhead->prev = NULL;
    memhead->next = NULL;
	PDEBUG(KERN_DEBUG "gememalloc_logic_init addr: 0x%08x\n", memhead->address) ;
