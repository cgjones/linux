...
DEFINE_SPINLOCK(ge_drv_lock);

#define MAX_CLIENTS 32
static uint64_t private_data[MAX_CLIENTS];
/*------------------------------------------------------------------------------
...
#endif
			clk_enable(ge_power_clk);
			clk_enable(ge_clk);
		PDEBUG("dev opened\n");
*/
void* temp = kmalloc(8,GFP_KERNEL);
filp->private_data = temp;
//printk("ge_drv open %s %d %x\n",current->comm,current->pid,temp);
	return 0;
}
...
static int ge_release(struct inode *inode, struct file *filp)
{
	...
	for(i =0; i<MAX_CLIENTS;i++)
	{
		if(private_data[i] == 0)
		{
			private_data[i]=*((uint64_t*)filp->private_data);
			break;
		}
	}
...
static long ge_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
		...
		case GE_IOC_RESERVE:
			spin_lock(&ge_drv_lock);
			__copy_from_user((filp->private_data), (void *)arg, 8);
		...
		case GE_IOC_WAIT: {
			...
			for(i =0; i<MAX_CLIENTS;i++)
			{
				if(private_data[i] != 0)
				{
					if(private_data[i] == filp->private_data)
					{
						private_data[i] = 0;
						break;
					}
					__copy_to_user((void *)arg, &(private_data[i]), 8);
					private_data[i] = 0;
					break;
				}
			}
...
#endif
{
	...
	irq_status_ge = readl(dev->hwregs +0x30);
	if (irq_status_ge & 0x1) {
		/* clear pp IRQ */
		writel(irq_status_ge | 0x00000004,
		       dev->hwregs +0x30);
...
//	ge_clk = clk_get(NULL, BCM_CLK_V3D_STR_ID);
//	ge_power_clk = clk_get(NULL, BCM_CLK_V3D_POWER_STR_ID);
//	clk_enable(ge_power_clk);
//	clk_enable(ge_clk);
...
/*	result = request_irq(IRQ_GRAPHICS, ge_drv_isr,
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,18))
	...
	alloc_mem = dma_alloc_coherent(NULL, 4096,
				       &dma_cohr_start_addr,
				       GFP_ATOMIC | GFP_DMA);

*/
//	clk_disable(ge_power_clk);
//	clk_disable(ge_clk);
...
#endif
	if (alloc_mem != NULL)
		dma_free_coherent(NULL, 4096, alloc_mem,
				  dma_cohr_start_addr);
	unregister_chrdev(ge_drv_major, "ge_drv");
